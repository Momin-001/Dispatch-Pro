 ...
 *
 * @param {string} permissionKey - e.g. "loads.create", "users.edit"
...
export function withPermission(permissionKey, handler) {
  return withAuth(async (request, context) => {
    ...
    if (!result || !result.enabled) {
      return errorResponse("Permission denied. Please refresh to access new permissions for your role.", 403 );
    }
